...
import { UploadResponseDto, BulkUploadResponseDto, BulkUploadResult } from './dto';
...
export class UploadService {
  ...
  async processBulkUpload(file: Express.Multer.File): Promise<BulkUploadResponseDto> {
    ...
    try {
      ...
      this.cleanupFile(file.path);
      throw new HttpException(`Bulk upload failed: ${error.message}`, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  private async parseExcel(filePath: string): Promise<ProductData[]> {
    try {
      ...
      if (jsonData.length < 2) {
        throw new HttpException('Excel file must contain headers and at least one data row', HttpStatus.BAD_REQUEST);
      }
      ...
      throw new HttpException(`Failed to parse Excel file: ${error.message}`, HttpStatus.BAD_REQUEST);
    }
  }
